...
import { PodcastConfig, PodcastResult, DebugTraceState, DebugStageLog, DebugStageId } from '../types/podcast.types';
...
export function usePodcastGeneration() {
  ...
  const generate = async (config: PodcastConfig) => {
    ...
    try {
      ...
      console.error('[PODCAST TRACE] Pipeline Error occurred:', err);
      const msg = err.message || 'Ошибка генерации сценария подкаста';
      setError(msg);
      toast.error(msg);

      // Find the currently active or pending stage to flag error
      const currentActive = debugTrace.stages.find(s => s.status === 'active');
      const errorStage: DebugStageId = currentActive ? currentActive.id : 'wait_response';
      
      updateStage(errorStage, { 
        status: 'error', 
        error: msg,
        details: `Пайплайн упал на этапе "${errorStage}". Ошибка: ${msg}`
      }, { 
        rawError: err.stack || err.toString(),
        totalDurationMs: Date.now() - startTime
      });

      // Mark other pending stages as error/cancelled
      setDebugTrace(prev => ({
        ...prev,
        stages: prev.stages.map(s => {
          if (s.id !== errorStage && s.status === 'pending') {
            return { ...s, status: 'pending', details: 'Отменено из-за ошибки' };
          }
          return s;
        })
      }));
